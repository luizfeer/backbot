...
module.exports = class Sessions {
    //
    static async start(app, sessionName) {
        ...
        if (session == false) {
            //create new session
            session = await Sessions.addSesssion(app, sessionName);
            ...
            Sessions.setup(app, sessionName);
    ...
    //
    //
    static async addSesssion(app, sessionName) {
    ...
    //
    //
    static async setup(app, sessionName) {
        ...
        if (session) {
            if (session.state == "CONNECTED") {
                var resultSendText = await session.client.then(async client => {
                    // Send basic text
                    return await client.sendText(number, text).then((result) => {
                        //console.log("Result: ", result); //return object success
